...
class Train(object):
    ...
    def construct_model(self):
        ...
        def task_metalearn(inp):
            ...
            task_lossb = tf.reduce_mean(tf.abs(labelb - task_outputb)) + 0.5 * tf.reduce_mean(lpips_tf.lpips(labelb, task_outputb, model='net-lin', net='alex')) + 0.1 * tf.reduce_mean(loss_func_gan(logits=task_fake_g_pred, labels=real)) + 0.002 * tf.reduce_mean((W-1)**2) / 2.0   + 0.5*tf.reduce_mean(self.ssl.ssl_loss_cal(task_outputb,labelb))
            ...
            task_lossesb.append(tf.reduce_mean(tf.abs(labelb - output)) + 0.5 * tf.reduce_mean(lpips_tf.lpips(labelb, output, model='net-lin', net='alex')) + 0.1 * tf.reduce_mean(loss_func_gan(logits=task_fake_g_pred_, labels=real)) + 0.002 * tf.reduce_mean((W-1)**2) / 2.0 +  0.5*tf.reduce_mean(self.ssl.ssl_loss_cal(output,labelb)) )
            ...
            for j in range(self.TASK_ITER - 1):
                ...
                task_outputbs.append(output)
                task_lossesb.append(tf.reduce_mean(tf.abs(labelb - output)) + 0.5 * tf.reduce_mean(lpips_tf.lpips(labelb, output, model='net-lin', net='alex')) + 0.1 * tf.reduce_mean(loss_func_gan(logits=fake_g_pred, labels=real)) + 0.002 * tf.reduce_mean((W-1)**2) / 2.0 +  0.5*tf.reduce_mean(self.ssl.ssl_loss_cal(output,labelb)) )
                task_lossesb_l1.append(tf.reduce_mean(tf.abs(labelb - output)))
                task_lossesb_vgg.append(tf.reduce_mean(lpips_tf.lpips(labelb, output, model='net-lin', net='alex')))
